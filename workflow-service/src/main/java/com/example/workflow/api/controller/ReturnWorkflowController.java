package com.example.workflow.api.controller;

import com.example.workflow.api.request.ReturnInspectionRequest;
import com.example.workflow.api.request.ReturnItemReceivedRequest;
import com.example.workflow.api.request.StartReturnWorkflowRequest;
import com.example.workflow.api.response.ReturnEventResponse;
import com.example.workflow.api.response.ReturnWorkflowResponse;
import com.example.workflow.application.service.ReturnWorkflowService;
import javax.validation.Valid;
import org.springframework.http.HttpStatus;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseStatus;
import org.springframework.web.bind.annotation.RestController;

@RestController
@RequestMapping("/api/workflows/returns")
public class ReturnWorkflowController {

    private final ReturnWorkflowService returnWorkflowService;

    public ReturnWorkflowController(
            ReturnWorkflowService returnWorkflowService
    ) {
        this.returnWorkflowService = returnWorkflowService;
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public ReturnWorkflowResponse start(
            @Valid @RequestBody StartReturnWorkflowRequest request
    ) {
        return returnWorkflowService.startReturnWorkflow(request);
    }

    @GetMapping("/{returnId}")
    public ReturnWorkflowResponse get(
            @PathVariable("returnId") String returnId
    ) {
        return returnWorkflowService.getReturnWorkflow(returnId);
    }

    @PostMapping("/{returnId}/received-events")
    public ReturnEventResponse receiveItem(
            @PathVariable("returnId") String returnId,
            @Valid @RequestBody ReturnItemReceivedRequest request
    ) {
        return returnWorkflowService.receiveReturnItem(
                returnId,
                request
        );
    }

    @PostMapping("/{returnId}/inspection-events")
    public ReturnEventResponse completeInspection(
            @PathVariable("returnId") String returnId,
            @Valid @RequestBody ReturnInspectionRequest request
    ) {
        return returnWorkflowService.completeInspection(
                returnId,
                request
        );
    }
}